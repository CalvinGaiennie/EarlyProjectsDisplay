...
// Read inputs and compute clicks using: MOASize = distance * 1.047 / 100, MOANum = inches / MOASize, numOfClicks = MOANum / MOAFactor
document.querySelector(".generate").addEventListener("click", function () {
  ...
  const MOASize = (distance * 1.047) / 100;
  const numOfClicksE = MOAFactor !== 0 ? (elevation / MOASize) / MOAFactor : 0;
  const numOfClicksW = MOAFactor !== 0 ? (windage / MOASize) / MOAFactor : 0;

  const msg = `Elevation: ${numOfClicksE.toFixed(1)} clicks. Windage: ${numOfClicksW.toFixed(1)} clicks.`;
  console.log(msg);
  const solutionEl = document.querySelector(".solutionText");
  if (solutionEl) solutionEl.textContent = msg;
});
